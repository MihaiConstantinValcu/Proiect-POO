...
Ecosistem::Ecosistem(int _suprafata, int _umiditate, float _temperatura, std::string _precipitatii, std::string inputAnimale, std::string inputPlante):suprafata(_suprafata), umiditate(_umiditate), temperatura(_temperatura),precipitatii(_precipitatii){
...
void Ecosistem::simulareSchimbareClima(const int _umiditate){
    if(this->umiditate < _umiditate){
        this->temperatura -= (_umiditate - this->umiditate)/2; // Daca umiditatea creste, atunci temperatura scade (formula fictiva)
        std::cout<<"Temperatura a scazut la: "<<this->temperatura<<".\n";
    }
    else if(this->umiditate > _umiditate){
        this->temperatura += (_umiditate - this->umiditate)/2; // Daca umiditatea scade, atunci temperatura creste (formula fictiva)
